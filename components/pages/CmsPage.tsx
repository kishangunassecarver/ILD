 ...
 */
const PROSE = [
  "text-[1.0625rem] leading-[1.75] text-ink-700",
  "[&>*:first-child]:mt-0",
  "[&_p]:mt-5",
  "[&_h2]:mt-10 [&_h2]:text-[1.375rem] [&_h2]:font-extrabold [&_h2]:leading-snug [&_h2]:tracking-tight [&_h2]:text-ink",
  ...
  "[&_a]:font-semibold [&_a]:text-brand-600 [&_a]:decoration-brand-200 [&_a]:decoration-2 [&_a]:underline-offset-[3px] [&_a]:underline hover:[&_a]:decoration-brand-500",
  "[&_strong]:font-bold [&_strong]:text-ink",
  "[&_blockquote]:mt-6 [&_blockquote]:border-l-[3px] [&_blockquote]:border-brand-500 [&_blockquote]:bg-paper [&_blockquote]:px-5 [&_blockquote]:py-4 [&_blockquote]:text-ink [&_blockquote]:rounded-r-lg [&_blockquote]:not-italic",
  ...
  "[&_th]:border-b [&_th]:border-line [&_th]:py-2.5 [&_th]:text-left [&_th]:font-bold [&_th]:text-ink",
          ...
          <article className="panel overflow-hidden">
            ...
            <div className="p-6 sm:p-9">
              {section && (
                <Link
                  href={`/${section.path}`}
                  className="inline-flex items-center gap-1.5 text-[0.6875rem] font-extrabold uppercase tracking-[0.14em] text-brand-500 transition hover:text-brand-600"
                >
              ...
              <h1 className="mt-3 text-[1.75rem] font-extrabold leading-[1.15] tracking-tight text-ink sm:text-[2.5rem]">
                {page.title}
              </h1>
...
function SectionPage({ page, trail }: { page: Page; trail: { label: string; href?: string }[] }) {
  return (
    <div className="shell py-6">
      ...
      {page.html && (
        <div className={cn("mb-10 max-w-[68ch]", PROSE)} dangerouslySetInnerHTML={{ __html: page.html }} />
      )}
...
function PrevNext({ previous, next }: { previous?: Page; next?: Page }) {
  return (
    <nav aria-label="More in this section" className="mt-4 grid gap-4 sm:grid-cols-2">
      {previous ? (
        <Link href={`/${previous.path}`} className="panel card-hover group flex items-center gap-3 p-4">
          <ArrowLeft className="h-4 w-4 shrink-0 text-muted transition group-hover:text-brand-500" aria-hidden />
          <span className="min-w-0">
            <span className="block text-[0.625rem] font-bold uppercase tracking-wider text-muted">
              Previous
            </span>
            <span className="mt-0.5 block truncate text-sm font-bold text-ink transition group-hover:text-brand-500">
              {previous.title}
            </span>
          </span>
        </Link>
      ...
      {next && (
        ...
        >
          <span className="min-w-0">
            <span className="block text-[0.625rem] font-bold uppercase tracking-wider text-muted">Next</span>
            <span className="mt-0.5 block truncate text-sm font-bold text-ink transition group-hover:text-brand-500">
              {next.title}
            </span>
          </span>
          <ArrowRight className="h-4 w-4 shrink-0 text-muted transition group-hover:text-brand-500" aria-hidden />
        </Link>
      )}
    </nav>
  );
...
function PopularListings() {
  ...
  return (
    ...
    >
      <h2
        id="popular-listings"
        className="border-b border-line bg-paper px-4 py-3 text-[0.6875rem] font-extrabold uppercase tracking-[0.14em] text-ink"
      >
        Popular in Durban
      </h2>
        ...
        {listings.map((listing) => (
          <li key={`${listing.hub}-${listing.slug}`}>
            <Link
              ...
              className="group flex items-center gap-3 p-3 transition hover:bg-paper"
            >
              <Tile
                seed={listing.slug}
                image={listing.image}
              ...
              <span className="min-w-0 flex-1">
                <span className="block truncate text-[0.8125rem] font-bold text-ink transition group-hover:text-brand-500">
                  {listing.name}
                </span>
      ...
      <Link
        href="/discover"
        className="flex items-center justify-between gap-2 border-t border-line px-4 py-3 text-xs font-semibold text-ink-700 transition hover:bg-paper hover:text-brand-500"
      >
        Explore Durban
        ...
        {page.children.map((child) => (
          ...
          >
            ...
            <div className="flex flex-1 flex-col p-4">
              ...
              <span className="mt-auto pt-3 text-[0.6875rem] font-bold uppercase tracking-wider text-brand-500 opacity-0 transition group-hover:opacity-100">
                Read more →
              </span>
            </div>
          </Link>
