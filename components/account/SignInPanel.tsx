...
export function SignInPanel() {
  ...
  if (state === "sent") {
    return (
      <div className="panel p-8 text-center">
        ...
        <button type="button" onClick={() => setState("idle")} className="btn-ghost mt-5">
  ...
  return (
    <form onSubmit={onSubmit} className="panel p-6 sm:p-8">
      ...
      <h2 className="text-base font-bold text-snow">Join or sign in</h2>
      ...
      <div className="mt-5 space-y-4">
        <div>
          <label htmlFor="signin-email" className="mb-1.5 block text-xs font-semibold text-snow">
            Email address <span className="text-aqua-300">*</span>
          </label>
          <input
            id="signin-email"
            type="email"
        ...
        <div>
          <label htmlFor="signin-name" className="mb-1.5 block text-xs font-semibold text-snow">
      ...
      {state === "error" && (
        <p role="alert" className="mt-3 text-xs font-medium text-brand-400">
          {message}
        </p>
      )}

      <p className="mt-4 flex items-start gap-2 text-[0.6875rem] leading-relaxed text-muted">
        <Heart className="mt-0.5 h-3.5 w-3.5 shrink-0 text-aqua-300" aria-hidden />
