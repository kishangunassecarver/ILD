...
export function SavedList() {
  ...
  if (loading) {
    return <p className="panel p-10 text-center text-sm text-muted" aria-busy>Loading your saves…</p>;
  }
  ...
  if (total === 0) {
    return (
      <div className="panel p-12 text-center">
        ...
        <h2 className="mt-4 text-base font-bold text-ink">Nothing saved yet</h2>
        <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-muted">
    ...
    <div className="space-y-10">
      ...
      {total < saves.length && (
        <p className="text-xs text-muted">
          {saves.length - total} saved{" "}
          {saves.length - total === 1 ? "item is" : "items are"} no longer listed, so they are not
          shown.
        </p>
      )}
    </div>
